...
describe('Chatbot request', () => {
    const chatbotRequest = new request_1.ChatbotRequest(chatbotRequestSample);
    describe('Bot Info', () => {
        it('get Bot Id', () => {
    ...
    describe('Action Info', () => {
        it('get skillIntentCode', () => {
            expect(chatbotRequest.skillIntentCode).toBe(chatbotRequestSample.action.params.skillIntentCode);
        });
    });
    describe('User Request Info', () => {
        it('get block id', () => {
            expect(chatbotRequest.blockId).toBe(chatbotRequestSample.userRequest.block.id);
        });
        it('get plusfriendUserKey', () => {
            expect(chatbotRequest.plusfriendUserKey).toBe(chatbotRequestSample.userRequest.user.properties.plusfriendUserKey);
        });
        it('get appUserId', () => {
            expect(chatbotRequest.appUserId).toBe(chatbotRequestSample.userRequest.user.properties.appUserId);
        });
        it('get callbackUrl', () => {
            expect(chatbotRequest.callbackUrl).toBe(chatbotRequestSample.userRequest.callbackUrl);
        });
    });
});
